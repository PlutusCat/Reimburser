...
class BaseWebViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if Layout.getSafeArea().bottom > 0.0 {
...
//        webView.addObserver(self, forKeyPath: "estimatedProgress", options: .new, context: nil)
        ...
        _ = webView.observe(\.isLoading, options: .new) { (webview, change) in
            printm("正在加载--")
        }
    ...
    @objc func popAction() {
        ...
        if let count = viewControllers?.count, count > 1 {
            if viewControllers![count-1] == self {
    ...
    override func viewWillLayoutSubviews() {
        ...
        let webY = Layout.getNavigationHeight(isFixed: true)
...
class NavigationBar: UIView {
    ...
    override func layoutSubviews() {
        ...
        let HHH = Layout.getNavigationHeight(isFixed: true)
        ...
        let itemY = Layout.getStatusBarFrame().maxY
        let itemW = Layout.getNavigationBarHeight()
        let itemX = Layout.getSafeArea().left + 8
